...
WITH CUSTOS AS (
    ...
    SELECT
        TCIE.ID AS ID_CUSTO_GERAL,
        TCP.ID AS ID_CUSTO,
        TCP.DESCRICAO AS CLASSIFICACAO_PRIMARIA,
        TCIE.DESCRICAO AS DESCRICAO_DA_DESPESA,
        TCIE.VALOR AS VALOR,
        CDC2.ID AS ID_CATEGORIA,
        CAST(CONCAT(YEAR(TCIE.DATA_COMPETENCIA), '-', MONTH(TCIE.DATA_COMPETENCIA), '-01') AS DATE) AS PRIMEIRO_DIA_MES_VENCIMENTO,
        TCIE.DATA_LANCAMENTO AS DATA_LANCAMENTO, -- Incluindo DATA_LANCAMENTO de TCIE
        TCIE.DATA_COMPETENCIA AS DATA_COMPETENCIA -- Incluindo DATA_COMPETENCIA de TCIE
    FROM
    ...
    SELECT
        TCCE.ID AS ID_CUSTO_GERAL, 
        TCP.ID AS ID_CUSTO,
        TCP.DESCRICAO AS CLASSIFICACAO_PRIMARIA,
        CONCAT(CDC.DESCRICAO, ' - ', TCP.DESCRICAO) AS DESCRICAO_DA_DESPESA,
        TCCE.VALOR AS VALOR,
        CDC2.ID AS ID_CATEGORIA,
        CAST(CONCAT(YEAR(TCCE.DATA_VENCIMENTO), '-', MONTH(TCCE.DATA_VENCIMENTO), '-01') AS DATE) AS PRIMEIRO_DIA_MES_VENCIMENTO,
        TCCE.DATA_LANCAMENTO AS DATA_LANCAMENTO, -- Incluindo DATA_LANCAMENTO de TCCE
        TCCE.DATA_PAGAMENTO AS DATA_COMPETENCIA -- Incluindo DATA_PAGAMENTO de TCCE
    FROM
...
# Consulta final
SELECT 
    CEC.ID_CUSTO_GERAL AS 'ID CUSTO',
    CEC.CLASSIFICACAO_PRIMARIA AS 'CLASSIFICAÇÃO PRIMÁRIA',
    CEC.DESCRICAO_DA_DESPESA AS 'DESCRIÇÃO DA DESPESA',
    CEC.VALOR AS 'VALOR',
    DATE_FORMAT(CEC.DATA_LANCAMENTO, '%d/%m/%Y') AS 'DATA LANÇAMENTO',
    DATE_FORMAT(CEC.DATA_COMPETENCIA, '%d/%m/%Y') AS 'DATA COMPETÊNCIA'
FROM CUSTOS CEC
...
JOIN 
    (SELECT CLASSIFICACAO_PRIMARIA, SUM(VALOR) AS TOTAL_VALOR
     ...
     GROUP BY CLASSIFICACAO_PRIMARIA
    ) AS TOTAL_CUSTOS
ON CEC.CLASSIFICACAO_PRIMARIA = TOTAL_CUSTOS.CLASSIFICACAO_PRIMARIA
